...
module Scripts
  def self.dump(path = "Data/Scripts", rxdata = "Data/Scripts.rxdata")
    ...
    folder_path = path
    folder_name = nil
    scripts.each_with_index do |e, i|   #### ITERAMOS SOBRE CADA SCRIPT ####
      _, title, script = e
      ...
      section_name = nil
      if title[/\[\[\s*(.+)\s*\]\]$/] || title.start_with?("=====")   # CREACIÓN DE CARPETA
        level       = 0    # Nivel de profundidad de la carpeta
        folder_path = path # Ruta principal
        folder_name = nil  # Limpiamos nombre de la carpeta

        section_name = $~[1].strip if title[/\[\[\s*(.+)\s*\]\]$/]
        section_name = "==================" if !section_name || section_name.empty?
        # Combinar número al nombre del script
        folder_num   = (i < scripts.length - 2) ? folder_id[level].to_digits(3) : "999"
        folder_name  = folder_num + "_" + section_name
        # Crear carpeta con ese nombre
        create_directory(folder_path + "/" + folder_name)
        # Indicamos que los siguientes scripts van dentro.
        folder_id[level] += 1
