...
module Compiler
  ...
  @@categories[:import_new_maps] = {
    ...
    :skipped_text   => proc { next _INTL("Ninguno encontrado") },
    :compile        => proc { import_new_maps }
  }

  @@categories[:create_missing_map_metadata] = {
    :should_compile => proc { |compiling| next true },
    :header_text    => proc { next _INTL("Creando metadatos de mapas para mapas que no los tienen") },
    :skipped_text   => proc { next _INTL("Ninguno encontrado") },
  ...
  @@categories[:map_data] = {
    :should_compile => proc { |compiling| next true },
    :header_text    => proc { next _INTL("Modificando eventos de mapas") },
    :skipped_text   => proc { next _INTL("No modificado") },
  ...
  @@categories[:messages] = {
    ...
    :header_text    => proc { next _INTL("Reuniendo mensajes para traducciones") },
    :skipped_text   => proc { next _INTL("No reunido") },
    :compile        => proc {
  ...
  def create_missing_map_metadata
    ...
    Console.echoln_li(_INTL("Metadatos creados para {1} mapa(s).", added_count))
