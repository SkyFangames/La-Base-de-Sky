...
module Compiler
      ...
      pbCompilerEachPreppedLine(path) do |line, line_no|
        ...
        if line[/^\s*\[\s*(.+)\s*\]\s*$/]
          ...
          if schema[key]   # Property of the animation
            ...
            if !current_particle
              raise _INTL("La partícula no ha sido definida todavía.") + "\n" + FileLineData.linereport
            end
  ...
  def validate_compiled_animation(hash)
    ...
    if hash[:particles].any? { |particle| particle[:name] == "User" } && hash[:no_user]
      raise _INTL("No se puede definir una partícula \"User\" y también establecer la propiedad \"NoUser\" en true.") + "\n" + FileLineData.linereport
    end
    # Ensure there is no "Target" particle if "NoTarget" is set
    if hash[:particles].any? { |particle| particle[:name] == "Target" } && hash[:no_target]
      raise _INTL("No se puede definir una partícula \"Target\" y también establecer la propiedad \"NoTarget\" en true.") + "\n" + FileLineData.linereport
    end
    ...
    hash[:particles].each do |particle|
      ...
      if hash[:no_user]
        if GameData::Animation::FOCUS_TYPES_WITH_USER.include?(particle[:focus])
          raise _INTL("La partícula \"{1}\" no puede tener un \"Focus\" que involucre a un usuario si la propiedad \"NoUser\" está establecida en true.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
        if ["USER", "USER_OPP", "USER_FRONT", "USER_BACK"].include?(particle[:graphic])
          raise _INTL("La partícula \"{1}\" no puede tener un \"Graphic\" que involucre a un usuario si la propiedad \"NoUser\" está establecida en true.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
        if particle[:name] == "SE" && particle[:user_cry] && !particle[:user_cry].empty?
          raise _INTL("La animación no puede reproducir el grito del usuario si la propiedad \"NoUser\" está establecida en true.") + "\n" + FileLineData.linereport
        end
      end
      ...
      if hash[:no_target]
        if GameData::Animation::FOCUS_TYPES_WITH_TARGET.include?(particle[:focus])
          raise _INTL("La partícula \"{1}\" no puede tener un \"Focus\" que involucre a un objetivo si la propiedad \"NoTarget\" está establecida en true.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
        if ["TARGET", "TARGET_OPP", "TARGET_FRONT", "TARGET_BACK"].include?(particle[:graphic])
          raise _INTL("La partícula \"{1}\" no puede tener un \"Graphic\" que involucre a un objetivo si la propiedad \"NoTarget\" está establecida en true.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
        if particle[:name] == "SE" && particle[:target_cry] && !particle[:target_cry].empty?
          raise _INTL("La animación no puede reproducir el grito del objetivo si la propiedad \"NoTarget\" está establecida en true.") + "\n" + FileLineData.linereport
        end
      end
      ...
      if GameData::Animation::FOCUS_TYPES_WITH_USER.include?(particle[:focus]) == GameData::Animation::FOCUS_TYPES_WITH_TARGET.include?(particle[:focus])
        if particle[:foe_invert_x]
          raise _INTL("La partícula \"{1}\" no puede establecer \"FoeInvertX\" si su foco no es exactamente 1 cosa.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
        if particle[:foe_invert_y]
          raise _INTL("La partícula \"{1}\" no puede establecer \"FoeInvertY\" si su foco no es exactamente 1 cosa.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
        if particle[:foe_flip]
          raise _INTL("La partícula \"{1}\" no puede establecer \"FoeFlip\" si su foco no es exactamente 1 cosa.",
                      particle[:name]) + "\n" + FileLineData.linereport
        end
      end
      ...
      if (particle[:angle_override] || :none) != :none &&
        ...
        raise _INTL("La partícula \"{1}\" no puede establecer \"AngleOverride\" si su foco no es una cosa(s) específica(s).",
                    particle[:name]) + "\n" + FileLineData.linereport
      end
        ...
        [:se, :user_cry, :target_cry].each do |property|
          ...
          particle[property].each do |play|
            ...
            if files_played[play[0]].include?(play[1])
              case property
              when :se
                raise _INTL("El SE \"{1}\" no debe reproducirse dos veces en el mismo fotograma ({2}).", play[1], play[0]) + "\n" + FileLineData.linereport
              when :user_cry
                raise _INTL("El grito del usuario no debe reproducirse dos veces en el mismo fotograma ({1}).", play[0]) + "\n" + FileLineData.linereport
              when :target_cry
                raise _INTL("El grito del objetivo no debe reproducirse dos veces en el mismo fotograma ({1}).", play[0]) + "\n" + FileLineData.linereport
              end
            end
      ...
      if particle[:blending]
        particle[:blending].each do |blend|
          next if blend[2] <= 2
          raise _INTL("Valor de blend inválido: {1} (debe ser 0, 1 o 2).\n{2}",
