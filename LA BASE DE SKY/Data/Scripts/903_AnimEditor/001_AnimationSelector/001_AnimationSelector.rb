...
class AnimationEditor::AnimationSelector
  ...
  def initialize_controls
    @components = UIControls::ListedContainer.new(0, 0, AnimationEditor::WINDOW_WIDTH, AnimationEditor::WINDOW_HEIGHT, @pop_up_viewport)
    # Quit button
    btn = UIControls::Button.new(QUIT_BUTTON_WIDTH, QUIT_BUTTON_HEIGHT, @viewport, _INTL("Quit"))
    @components.add_control_at(:quit, 0, 0, btn)
    # New button
    btn = UIControls::Button.new(QUIT_BUTTON_WIDTH, QUIT_BUTTON_HEIGHT, @viewport, _INTL("New"))
    @components.add_control_at(:new, QUIT_BUTTON_WIDTH, 0, btn)
    # Type label
    label = UIControls::Label.new(TYPE_BUTTON_WIDTH, TYPE_BUTTON_HEIGHT, @viewport, _INTL("Anim types"))
    label.header = true
    @components.add_control_at(:type_label, TYPE_BUTTONS_X + LABEL_OFFSET_X + 4, TYPE_BUTTONS_Y + LABEL_OFFSET_Y + 4, label)
    ...
    [[:moves, _INTL("Moves")], [:commons, _INTL("Common")]].each_with_index do |val, i|
      ...
      @components.add_control_at(val[0], TYPE_BUTTONS_X, TYPE_BUTTONS_Y + (i * TYPE_BUTTON_HEIGHT), btn)
    end
    # Moves list label
    label = UIControls::Label.new(MOVES_LIST_WIDTH, TYPE_BUTTON_HEIGHT, @viewport, _INTL("Moves"))
    label.header = true
    @components.add_control_at(:moves_label, MOVES_LIST_X + LABEL_OFFSET_X, MOVES_LIST_Y + LABEL_OFFSET_Y, label)
    # Moves list
    ...
    label.header = true
    @components.add_control_at(:animations_label, ANIMATIONS_LIST_X + LABEL_OFFSET_X, ANIMATIONS_LIST_Y + LABEL_OFFSET_Y, label)
    # Animations list
    ...
    [[:edit, _INTL("Edit animation")], [:copy, _INTL("Copy animation")], [:delete, _INTL("Delete animation")]].each_with_index do |val, i|
      ...
      @components.add_control_at(val[0], ACTION_BUTTON_X, ACTION_BUTTON_Y + (i * ACTION_BUTTON_HEIGHT), btn)
    end
    # Filter text box
    text_box = UIControls::TextBox.new(FILTER_BOX_WIDTH, FILTER_BOX_HEIGHT, @viewport, "")
    @components.add_control_at(:filter, FILTER_BOX_X, FILTER_BOX_Y, text_box)
    # Filter text box label
    label = UIControls::Label.new(FILTER_BOX_WIDTH, TYPE_BUTTON_HEIGHT, @viewport, _INTL("Filter text"))
    label.header = true
    @components.add_control_at(:filter_label, FILTER_BOX_X + LABEL_OFFSET_X, FILTER_BOX_Y + LABEL_OFFSET_Y, label)
  end
  ...
  def draw_editor_background
    ...
    @screen_bitmap.bitmap.fill_rect(0, 0, AnimationEditor::WINDOW_WIDTH, AnimationEditor::WINDOW_HEIGHT, get_color_of(:background))
    # Make the pop-up background semi-transparent
    @pop_up_bg_bitmap.bitmap.fill_rect(0, 0, AnimationEditor::WINDOW_WIDTH, AnimationEditor::WINDOW_HEIGHT, get_color_of(:semi_transparent))
  end
