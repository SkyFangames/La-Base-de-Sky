...
class UI::SavePanel < UI::SpriteContainer
  ...
  def initialize_player_sprite
    ...
    if !@sprites[:player].bitmap
      raise _INTL("No se ha encontrado el charset del jugador {1} andando (archivo: \"{2}\").", 
  ...
  def draw_save_file_text
    if !@save_data
      draw_text(_INTL("Crear un nuevo archivo de guardado"), width / 2, (height / 2) - 10, align: :center, theme: :gray)
      return
    end
...
class UI::SaveVisuals < UI::BaseVisuals
  ...
  def refresh_overlay
    ...
    if @choosing_save_file
      if @save_data[index] &&
        ...
        if @save_data[index][1][:game_system].adventure_magic_number == $game_system.adventure_magic_number
          ...
          if delta_time >= 0
            ...
            if hour > 0
              draw_text(_INTL("Tiempo de juego desde el último guardado: {1}h {2}m", hour, min), 8, 4, theme: :gray)
            else
              draw_text(_INTL("Tiempo de juego desde el último guardado: {1}m", min), 8, 4, theme: :gray)
            end
          else
            draw_text(_INTL("¡Versión alternativa de tu aventura!"), 8, 4, theme: :gray)
          end
        else
          draw_text(_INTL("¡Aventura diferente!"), 8, 4, theme: :gray)
        end
      end
      if @save_data[@index]
      ...
      draw_text(_INTL("Último guardado el {1} a las {2}", date_text, time_text), 8, 4, theme: :gray)
    end
  end
  ...
  def navigate
    help_text = _INTL("Elige un archivo para guardar.")
...
class UI::Save < UI::BaseScreen
  ...
  def prompt_overwrite_save_file(slot_index)
    if different_adventure?(slot_index)
      show_message(_INTL("¡AVISO!") + "\1")
      show_message(_INTL("Tienes ya un archivo de guardado de una partida diferente.") + "\1")
      show_message(_INTL("Si guardas ahora, todos los datos de la otra partida se perderán para siempre.") + "\1")
      if !show_confirm_serious_message(_INTL("¿Estás segur{1} de que quieres guardar la partida y sobreescribir el otro archivo?", $player.female? ? 'a' : 'o'))
        return false
      end
    end
    return true
  ...
  def save_game(file_number)
    if Game.save(file_number)
      ...
      show_message(_INTL("El guardado ha fallado."))
      @result = false
    end
  end
  ...
  def main
    ...
    when :multiple
      ...
      loop do
        ...
        if !@save_data[index] ||
           show_confirm_message(_INTL("¿Quieres sobreescribir esta partida?"))
          if different_adventure?(index)
            show_message(_INTL("¡AVISO!") + "\1")
            pbPlayDecisionSE
            show_message(_INTL("Este archivo de guardado es de una aventura diferente.") + "\1")
            pbPlayDecisionSE
            show_message(_INTL("Si guardas ahora, esa aventura, incluidos los objetos y Pokémon, se perderá por completo.") + "\1")
            pbPlayDecisionSE
            next if !show_confirm_serious_message(_INTL("¿Estás segur{1} de que quieres sobreescribirlo?", $player.female? ? 'a' : 'o'))
          end
...
def pbEmergencySave
  ...
  if Game.save(filename_number)
    ...
    pbMessage("\\se[]" + _INTL("El archivo de guardado anterior ha sido respaldado.") + "\\wtnp[20]")
  else
