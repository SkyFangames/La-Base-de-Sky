module UI::LoadSaveDataMixin
  ...
  def load_save_file(directory, filename)
    ...
    if !SaveData.valid?(ret)
      if File.file?(directory + filename + ".bak")
        show_message(_INTL("La partida guardada está corrupta. Se cargará una copia de seguridad."))
  ...
  def prompt_corrupted_save_deletion(filename)
    show_message(_INTL("La partida guardada está corrupta o es incompatible con este juego.") + "\1")
    pbPlayDecisionSE
    return show_confirm_serious_message(_INTL("¿Quieres eliminar la partida guardada y empezar de nuevo?"))
  end
...
class UI::LoadContinuePanel < UI::LoadPanel
  ...
  def set_player_sprite
    ...
    if !@sprites[:player].bitmap
      raise _INTL("No se ha encontrado el charset del jugador {1} andando (archivo: \"{2}\").", @save_data[:player].character_ID, filename)
    end
  end
  ...
  def draw_save_file_text
    ...
    # Gym Badges
    draw_text(_INTL("Medallas:"), 18, 146, theme: :gray)
    ...
    # Time played
    draw_text(_INTL("Tiempo:"), 18, 210, theme: :gray)
...
class UI::Load < UI::BaseScreen
  ...
  def prompt_save_deletion(filename)
    if show_confirm_serious_message(_INTL("¿Quieres borrar la partida guardada?"))
      show_message(_INTL("Una vez que la partida es borrada, no hay forma de recuperarla.") + "\1")
      pbPlayDecisionSE
      if show_confirm_serious_message(_INTL("¿Estás seguro que deseas borrar la partida guardada de todas formas?"))
        delete_save_data(filename) {
...
MenuHandlers.add(:load_screen, :update, {
  "name"      => _INTL("Buscar Actualizaciones"),
  "order"     => 50,
  "condition" => proc { |screen| next PluginManager.installed?("Pokemon Essentials Game Updater") }
})
