module EventReporting
  ...
  def choose_print
    ...
    return pbMessage(_INTL("¿Dónde mostrar los resultados?"), commands_type, -1, nil, 0)
  end

  def print_info(location, lines, path, append = false)
    case location
    when :Console
      lines.each {|line| echoln line}
    when :File
      Dir.mkdir("Event Reports") rescue nil if !FileTest.directory?("Event Reports")
      path = "Event Reports/" + path
      if append
        if Essentials::VERSION.include?("20") 
          if safeExists?(path)
            ...
            n = 1
            loop do
              newpath = File.join(dir, "#{base}_#{n}#{ext}")
      ...
      pbMessage(_INTL("Resultados guardados en {1}.", path))
      return true
    end
  end
end
